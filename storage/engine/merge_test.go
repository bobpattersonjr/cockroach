...
func TestGoMerge(t *testing.T) {
	...
	for i, c := range testCasesTimeSeries {
		...
		// Test the C++ implementation of merging first using goMerge.  goMerge
		// operates directly only marshalled bytes.
		result, err := goMerge(c.existing, c.update)
		if err != nil {
		...
		// Test the Go implementation of merging.
		if existingTS == nil {
			resultTS, err = proto.MergeInternalTimeSeriesData(updateTS)
		} else {
			resultTS, err = proto.MergeInternalTimeSeriesData(existingTS, updateTS)
		}
		if err != nil {
